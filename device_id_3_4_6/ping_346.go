...
func main() {
	...
	rows, err := mysqlDB.Query("SELECT id, ip FROM ip_monitor WHERE device_id IN (3, 4, 6)")
	if err != nil {
	...
	if len(ips) == 0 {
		log.Fatal("Tidak ada IP yang ditemukan untuk device_id IN (3, 4, 6)")
	}
	...
	for {
		select {
		case <-ticker.C:
			pingWithConcurrency(ips, sqliteDB, 100) // Batasi 10 goroutine secara konkuren
		}
	}
}
